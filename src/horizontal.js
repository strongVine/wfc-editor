...
(function () {
  ...
  // Уведомить потребителей (вкладка «Гор. визуал») о структурных
  ...
  // ------------------------------------------------------------
  // Вкладка «Гор. визуал» редактирует тот же файл через эти точки. По сути
  ...
  W.initHorizontal = function () {
    ...
    // Ctrl+S работает и в горизонтали, и в её визуале — визуал редактирует
    // тот же файл, что и горизонталь, поэтому отдельный хоткей не нужен.
    document.addEventListener('keydown', (e) => {
      const mode = W.getCurrentMode();
      if (mode !== 'horizontal' && mode !== 'horizontal-visual') return;
